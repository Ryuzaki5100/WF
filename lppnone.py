...
nodes = ['USD', 'Exchange1', 'Exchange2', 'Exchange3']
edges = {
    ...
    ('Exchange3', 'USD'): {'profit': 1.07, 'fee': 0.02, 'risk': 0.01},
...
for node in nodes:
    if node == 'USD':
        prob += pulp.lpSum([f[('USD', j)] for j in nodes if ('USD', j) in edges]) == 1000  # initial USD amount
        prob += pulp.lpSum([f[(i, 'USD')] for i in nodes if (i, 'USD') in edges]) == 1000  # initial USD amount + desired profit
    else:
